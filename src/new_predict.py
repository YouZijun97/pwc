...
data_dir_path = DIR / "model" / "命名实体识别train9_中文106.model"

model_load = torch.load(data_dir_path, map_location="cpu")

# model_load = torch.load('./model/命名实体识别_中文10.model', map_location='cpu')
model_load.eval()
...
def outp(sentence):
    ...
    tag = []
    input_id2 = []
    for part_sentence in sentence_list:
        if part_sentence == "":
            continue

        input_id1, part_tag = part_sentence_output(part_sentence)
        input_id2 += input_id1
        tag += part_tag
    ...
    return entity_list, person_list, description_list2


def refine_entity(entity_list):
    entity2name = dict()
    for entity in entity_list:
        ...
        if x != {}:
            y = max(x, key=x.get)
            entity2name[entity] = y
        else:
            continue
    return entity2name


def entity_find2(sentence, id):
    """
    :param sentence:
    :return:
    """
    ...
    # print(entity_list)
    entity_name_dict = refine_entity(entity_list)
    for ke in entity_name_dict.keys():
        de = []
        for description in description_list2:
            if near_part(ke, description, sentence):
                de.append(description)
        a = Entity(id, entity_name_dict[ke], ke, sentence, de)
        a.show_entity()
        id += 1
        answer.append(a)
    return answer, id
...
def near_part(str1, str2, sentence, distance=3):
    """
    判断str1，str2是否邻近
    :param str1:
    :param str2:
